...
CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go test -c -tags=integration -o "${OUT_DIR}/itest-manifest-linux-amd64" ./tests/integration
...
CGO_ENABLED=0 GOOS=windows GOARCH=amd64 go test -c -tags=integration -o "${OUT_DIR}/itest-manifest-windows-amd64.exe" ./tests/integration
