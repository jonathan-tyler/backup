...
BIN_PATH="$(mktemp /tmp/backup-integration-links.XXXXXX.test)"
cleanup() {
  rm -f "${BIN_PATH}"
}
trap cleanup EXIT
...
echo
echo "Running WSL links integration test with pause enabled..."
echo "Press Enter in the test prompt to continue cleanup."
echo "Using backup binary: ${BACKUP_BINARY}"
echo

"${BIN_PATH}" -test.v -test.run TestIntegrationWSLLinksFlow
